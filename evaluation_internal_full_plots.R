...
probs_df <- data.frame(
  ...
  Logistic = log_probs$Yes,
  KNN = knn_probs$Yes,
  RandomForest = rf_probs$Yes,
  SVM = SVM_probs$Yes,
  XGBoost = xgb_probs$Yes,
  DecisionTree = tree_probs$Yes
)
...
pr_long_df <- probs_df %>%
  pivot_longer(cols = all_of(model_names), names_to = "Model", values_to = "Prob")
...
plot_pr <- ggplot(pr_data, aes(x = recall, y = precision, color = Model)) +
  geom_line(size = 1) +
...
plot_pr
plot_pr_validation <- plot_pr
ggsave("Figures/plot_pr_validarion.tiff",
       width = 7,
       height = 5,
       units = "in",
       dpi = 300)
...
for(m in model_names){
  cal_list[[m]] <- get_calibration_data(probs_df[[m]], probs_df$obs, m)
}
library(dplyr)
...
cal_df$Model <- as.character(cal_df$Model)
cal_df$Model[cal_df$Model == "RandomForest"] <- "Random Forest"
cal_df$Model[cal_df$Model == "DecisionTree"] <- "Decision Tree"

cal_df$Model <- factor(cal_df$Model, levels = target_levels)
plot_cal_validation <- ggplot(cal_df, aes(x = midpoint, y = Percent, color = Model)) +
...
ggsave("Figures/plot_cal_validation.tiff",
       width = 7,
       height = 5,
       units = "in",
       dpi = 300)
...
table(dca_df$label)
plot_dca <- ggplot(dca_df, aes(x = threshold, y = net_benefit, color = label,  linetype = label)) +
  geom_line(size = 1.0) +
...
plot_dca
plot_dca_validation <- plot_dca

ggsave("Figures/plot_dca_validation.tiff",
       width = 7,
       height = 5,
       units = "in",
       dpi = 300)
...
for (m in model_names) {
  ...
  cic_plot_data_list[[m]] <- get_cic_bootstrap(probs_df, model_col = m, obs_col = "obs", n_boot = 500)
}
...
ggplot(cic_final_df, aes(x = Threshold)) +

  geom_ribbon(aes(ymin = Risk_Lower, ymax = Risk_Upper), fill = "#E64B35FF", alpha = 0.2) +
  geom_line(aes(y = Risk_Mean), color = "#E64B35FF", size = 0.8) +

  geom_ribbon(aes(ymin = TP_Lower, ymax = TP_Upper), fill = "#4DBBD5FF", alpha = 0.2) +
  geom_line(aes(y = TP_Mean), color = "#4DBBD5FF", size = 0.8) +

  geom_vline(data = intersection_df, aes(xintercept = Threshold),
             linetype = "dashed", color = "black", size = 0.6) +

  geom_text(data = intersection_df,
            aes(x = Threshold, y = 900, label = paste0("T = ", Threshold)),
            hjust = -0.1,
            vjust = 0,
  ...
  labs(
    title = "REDACTED",
    subtitle = "Vertical line indicates the threshold where 95% CIs of predicted vs. true positives first overlap.",
    x = "Threshold Probability",
...
plot_cic_facet_xgb <- ggplot(cic_final_df_xgb, aes(x = Threshold)) +
  ...
  geom_text(
    aes(x = 0.02, y = 900, label = paste0("T = ", 0.02)),
    hjust = -1,
    vjust = -0.2,
    family = "Times New Roman", size = 3.5, fontface = "bold") +
...
plot_cic_facet_xgb
ggsave("Figures/plot_cic_facet_validation_xgb.tiff",
       width = 7,
       height = 5,
       units = "in",
       dpi = 300)
...
intersection_df
plot_cic_facet_rf <- ggplot(cic_final_df_rf, aes(x = Threshold)) +
...
plot_cic_facet_rf
ggsave("Figures/plot_cic_facet_validation_rf.tiff",
       width = 7,
       height = 5,
       units = "in",
       dpi = 300)
...
intersection_df
plot_cic_facet_lg <- ggplot(cic_final_df_lg, aes(x = Threshold)) +
  ...
  geom_text(
    aes(x = 0.5, y = 900, label = paste0("T = ", 0.28)),
    hjust = 1,
    vjust = -1,
    family = "Times New Roman", size = 3.5, fontface = "bold") +
...
plot_cic_facet_lg
ggsave("Figures/plot_cic_facet_validation_lg.tiff",
       width = 7,
       height = 5,
       units = "in",
       dpi = 300)
...
intersection_df
plot_cic_facet_knn <- ggplot(cic_final_df_knn, aes(x = Threshold)) +
  ...
  geom_text(
    aes(x = 0.4, y = 900, label = paste0("T = ", 0.12)),
    hjust = 1,
    vjust = -1,
    family = "Times New Roman", size = 3.5, fontface = "bold") +
...
plot_cic_facet_knn
ggsave("Figures/plot_cic_facet_validation_knn.tiff",
       width = 7,
       height = 5,
       units = "in",
       dpi = 300)
...
intersection_df
plot_cic_facet_svm <- ggplot(cic_final_df_svm, aes(x = Threshold)) +
...
plot_cic_facet_svm
ggsave("Figures/plot_cic_facet_validation_svm.tiff",
       width = 7,
       height = 5,
       units = "in",
       dpi = 300)
...
intersection_df
plot_cic_facet_dt <- ggplot(cic_final_df_dt, aes(x = Threshold)) +
...
plot_cic_facet_dt
ggsave("Figures/plot_cic_facet_validation_dt.tiff",
       width = 7,
       height = 5,
       units = "in",
       dpi = 300)
...
DDDDDDEEEEEEFFFFFF
'
plot_cic_facet_rf + plot_cic_facet_lg + plot_cic_facet_dt + plot_cic_facet_svm +plot_cic_facet_knn+guide_area() +
  plot_layout(guides = 'collect', design = layout)
ggsave("Figures/plot_cic_facet_validation.tiff",
       width = 18,
       height = 8,
       units = "in",
       dpi = 300)
...
plotmodel_validation_set
ggsave("Figures/plot_ROC_validation.tiff",
       width = 7,
       height = 5,
       units = "in",
       dpi = 300)
...
library(caret)
xgb_model_raw <- model_xgb$finalModel

predictors_data_for_shap <- data_predictors_af_logistic_have_ACD_LT_train %>%
  select(-Group)
...
library(shapviz)
library(kernelshap)
shap_values_obj <- shapviz( model_xgb$finalModel, X_pred = X_train_matrix)
colnames(shap_values_obj$X)
new_names_vector <- c("Sphere", "Cylinder", "BCVA (logMAR)", "AL", "K1", "ACD", "LT", "K2")
...
p
ggsave("Figures/sv_importance_xgb.tiff",
       width = 7,
       height = 5,
       units = "in",
       dpi = 300)
