...
coef_plot_noacdlt
ggsave("Figures/Lasso_coef_plot_noacdlt.tiff",
       width = 7,
       height = 5,
       units = "in",
       dpi = 300)

logistic_model_varible_select_noacdlt<- glm(Group ~  Age+ gender+ pre_S+ pre_C+LogMAR+AL+ K1+K2+WTW,
                                    data = training_set,
                                    family = binomial(link = "logit"))
summary(logistic_model_varible_select_noacdlt)

library(gtsummary)
library(flextable)

table_output <- logistic_model_varible_select_noacdlt %>%
  tbl_regression(
    exponentiate = FALSE,
...
coef_plot_noacd <- ggplot(coef_long, aes(x = log_lambda, y = Coefficient, group = Feature)) +
...
coef_plot_noacd
ggsave("Figures/Lasso_coef_plot_noacd.tiff",
       width = 7,
       height = 5,
       units = "in",
       dpi = 300)

logistic_model_varible_select_noacd<- glm(Group ~  Age+ gender+ pre_S+ pre_C+LogMAR+AL+ K1+K2+LT,
                                            data = training_set,
                                            family = binomial(link = "logit"))
summary(logistic_model_varible_select_noacd)

library(gtsummary)
library(flextable)

table_output <- logistic_model_varible_select_noacd %>%
  tbl_regression(
    exponentiate = FALSE,
    pvalue_fun = ~style_pvalue(.x, digits = 3)
  ) %>%
...
coef_plot_nolt <- ggplot(coef_long, aes(x = log_lambda, y = Coefficient, group = Feature)) +
...
coef_plot_nolt
ggsave("Figures/Lasso_coef_plot_nolt.tiff",
       width = 7,
       height = 5,
       units = "in",
       dpi = 300)

logistic_model_varible_select_nolt<- glm(Group ~  Age+ gender+ pre_S+ pre_C+LogMAR+AL+ K1+K2+ACD+WTW,
                                          data = training_set,
                                          family = binomial(link = "logit"))
summary(logistic_model_varible_select_nolt)

library(gtsummary)
library(flextable)

table_output <- logistic_model_varible_select_nolt %>%
  tbl_regression(
    exponentiate = FALSE,
    pvalue_fun = ~style_pvalue(.x, digits = 3)
  ) %>%
...
save_as_docx(table_output, path = "tables/Logistic_Model_Result_nolt.docx")
