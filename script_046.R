...
probs_df_no_ACD_LT <- data.frame(
  obs = validation_set_moreplot$Group,
  obs_num = actual_outcome,
  Logistic = log_probs_no_ACD_LT$Yes,
  KNN = knn_probs_no_ACD_LT$Yes,
  RandomForest = rf_probs_no_ACD_LT$Yes,
  SVM = SVM_probs_no_ACD_LT$Yes,
  XGBoost = xgb_probs_no_ACD_LT$Yes,
  DecisionTree = tree_probs_no_ACD_LT$Yes
)

model_names <- c("Logistic", "KNN", "RandomForest", "SVM", "XGBoost", "DecisionTree")

pr_long_df <- probs_df_no_ACD_LT %>%
...
plot_pr_no_ACD_LT <- ggplot(pr_data, aes(x = recall, y = precision, color = Model)) +
  geom_line(size = 1) +
...
ggsave("Figures/plot_pr_validarion_no_ACD_LT.tiff",
       width = 7,
       height = 5,
       units = "in",
       dpi = 300)
...
for(m in model_names){
  cal_list[[m]] <- get_calibration_data(probs_df_no_ACD_LT[[m]], probs_df_no_ACD_LT$obs, m)
}
library(dplyr)
cal_df_no_ACD_LT <- bind_rows(cal_list)

cal_df_no_ACD_LT
target_levels <- c("Logistic", "KNN", "Random Forest", "SVM", "XGBoost", "Decision Tree")

cal_df_no_ACD_LT$Model <- as.character(cal_df_no_ACD_LT$Model)
cal_df_no_ACD_LT$Model[cal_df_no_ACD_LT$Model == "RandomForest"] <- "Random Forest"
cal_df_no_ACD_LT$Model[cal_df_no_ACD_LT$Model == "DecisionTree"] <- "Decision Tree"

cal_df_no_ACD_LT$Model <- factor(cal_df_no_ACD_LT$Model, levels = target_levels)
plot_cal_validation_no_ACD_LT <- ggplot(cal_df_no_ACD_LT, aes(x = midpoint, y = Percent, color = Model)) +
...
plot_cal_validation_no_ACD_LT

ggsave("Figures/plot_cal_validation_no_ACD_LT.tiff",
       width = 7,
       height = 5,
       units = "in",
       dpi = 300)
...
table(dca_df$label)
plot_dca_no_ACD_LT <- ggplot(dca_df, aes(x = threshold, y = net_benefit, color = label,  linetype = label)) +
  geom_line(size = 1.0) +
...
plot_dca_no_ACD_LT
plot_dca_validation_no_ACD_LT <- plot_dca_no_ACD_LT

ggsave("Figures/plot_dca_validation_no_ACD_LT.tiff",
       width = 7,
       height = 5,
       units = "in",
       dpi = 300)
...
for (m in model_names) {
  ...
  cic_plot_data_list[[m]] <- get_cic_bootstrap(probs_df_no_ACD_LT,
...
plot_cic_facet_xgb_no_ACD_LT <- ggplot(cic_final_df_xgb, aes(x = Threshold)) +
  ...
  geom_text(
    aes(x = 0.1, y = 900, label = paste0("T = ", 0.12)),
    hjust = -1,
    vjust = -0.2,
...
plot_cic_facet_xgb_no_ACD_LT
ggsave("Figures/plot_cic_facet_validation_xgb_no_ACD_LT.tiff",
       width = 7,
       height = 5,
       units = "in",
       dpi = 300)
...
intersection_df
plot_cic_facet_rf_no_ACD_LT <- ggplot(cic_final_df_rf, aes(x = Threshold)) +
  ...
  geom_text(
    aes(x = 0.4, y = 900, label = paste0("T = ", 0.26)),
    hjust = 1,
    vjust = -1,
    family = "Times New Roman", size = 3.5, fontface = "bold") +
...
plot_cic_facet_rf_no_ACD_LT
ggsave("Figures/plot_cic_facet_validation_rf_no_ACD_LT.tiff",
       width = 7,
       height = 5,
       units = "in",
       dpi = 300)
...
intersection_df
plot_cic_facet_lg_no_ACD_LT <- ggplot(cic_final_df_lg, aes(x = Threshold)) +
  ...
  geom_vline(aes(xintercept = 0.38),
             linetype = "dashed", color = "black", size = 0.6) +

  geom_text(
    aes(x = 0.35, y = 900, label = paste0("T = ", 0.38)),
    hjust = 1,
    vjust = -1,
    family = "Times New Roman", size = 3.5, fontface = "bold") +
...
plot_cic_facet_lg_no_ACD_LT
ggsave("Figures/plot_cic_facet_validation_lg_no_ACD_LT.tiff",
       width = 7,
       height = 5,
       units = "in",
       dpi = 300)
...
intersection_df
plot_cic_facet_knn_no_ACD_LT <- ggplot(cic_final_df_knn, aes(x = Threshold)) +
...
plot_cic_facet_knn_no_ACD_LT
ggsave("Figures/plot_cic_facet_validation_knn_no_ACD_LT.tiff",
       width = 7,
       height = 5,
       units = "in",
       dpi = 300)
...
intersection_df
plot_cic_facet_svm_no_ACD_LT <- ggplot(cic_final_df_svm, aes(x = Threshold)) +
  ...
  geom_text(
    aes(x = 0.2, y = 900, label = paste0("T = ", 0.06)),
    hjust = 1,
    vjust = -1,
    family = "Times New Roman", size = 3.5, fontface = "bold") +
...
plot_cic_facet_svm_no_ACD_LT
ggsave("Figures/plot_cic_facet_validation_svm_no_ACD_LT.tiff",
       width = 7,
       height = 5,
       units = "in",
       dpi = 300)
...
intersection_df
plot_cic_facet_dt_no_ACD_LT <- ggplot(cic_final_df_dt, aes(x = Threshold)) +
...
plot_cic_facet_dt_no_ACD_LT
ggsave("Figures/plot_cic_facet_validation_dt_no_ACD_LT.tiff",
       width = 7,
       height = 5,
       units = "in",
       dpi = 300)
...
DDDDDDEEEEEEFFFFFF
'
plot_cic_facet_rf_no_ACD_LT + plot_cic_facet_lg_no_ACD_LT +
  plot_cic_facet_dt_no_ACD_LT + plot_cic_facet_svm_no_ACD_LT +plot_cic_facet_knn_no_ACD_LT+guide_area() +
  plot_layout(guides = 'collect', design = layout)
ggsave("Figures/plot_cic_facet_validation_no_ACD_LT.tiff",
       width = 18,
       height = 8,
       units = "in",
       dpi = 300)
...
plotmodel_validation_set_no_ACD_LT <- ggplot(roc_no_ACD_LT_df, aes(x = FPR, y = TPR, color = Model)) +
...
plotmodel_validation_set_no_ACD_LT
ggsave("Figures/plot_ROC_validation_no_ACD_LT.tiff",
       width = 7,
       height = 5,
       units = "in",
       dpi = 300)

xgb_model_raw <- model_no_ACD_LT_xgb$finalModel

predictors_data_for_shap <- data_predictors_af_logistic_no_ACD_LT_train %>%
  select(-Group)

X_train_matrix <- as.matrix(predictors_data_for_shap )

shap_values_obj <- shapviz( model_no_ACD_LT_xgb$finalModel, X_pred = X_train_matrix)
colnames(shap_values_obj$X)
new_names_vector <- c("Sphere", "Cylinder", "BCVA (logMAR)", "AL", "K1", "K2")
...
p
ggsave("Figures/sv_importance_xgb_no_ACD_LT.tiff",
       width = 7,
       height = 5,
       units = "in",
       dpi = 300)
